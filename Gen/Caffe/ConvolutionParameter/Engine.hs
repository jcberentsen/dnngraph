...
module Gen.Caffe.ConvolutionParameter.Engine (Engine(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Engine where
  ...
  fromEnum CUDNN = 2
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Gen.Caffe.ConvolutionParameter.Engine") .
      toMaybe'Enum
  succ DEFAULT = CAFFE
  succ CAFFE = CUDNN
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Gen.Caffe.ConvolutionParameter.Engine"
  pred CAFFE = DEFAULT
  pred CUDNN = CAFFE
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Gen.Caffe.ConvolutionParameter.Engine"
...
instance P'.ReflectEnum Engine where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".caffe.ConvolutionParameter.Engine") ["Gen"] ["Caffe", "ConvolutionParameter"] "Engine")
      ["Gen", "Caffe", "ConvolutionParameter", "Engine.hs"]
