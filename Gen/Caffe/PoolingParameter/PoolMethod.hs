...
module Gen.Caffe.PoolingParameter.PoolMethod (PoolMethod(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum PoolMethod where
  ...
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Gen.Caffe.PoolingParameter.PoolMethod") .
      toMaybe'Enum
  succ MAX = AVE
  succ AVE = STOCHASTIC
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Gen.Caffe.PoolingParameter.PoolMethod"
  pred AVE = MAX
  pred STOCHASTIC = AVE
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Gen.Caffe.PoolingParameter.PoolMethod"
...
instance P'.ReflectEnum PoolMethod where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".caffe.PoolingParameter.PoolMethod") ["Gen"] ["Caffe", "PoolingParameter"] "PoolMethod")
      ["Gen", "Caffe", "PoolingParameter", "PoolMethod.hs"]
