...
module Gen.Caffe.ReLUParameter.Engine (Engine(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Engine where
  ...
  fromEnum CUDNN = 2
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Gen.Caffe.ReLUParameter.Engine") . toMaybe'Enum
  succ DEFAULT = CAFFE
  succ CAFFE = CUDNN
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Gen.Caffe.ReLUParameter.Engine"
  pred CAFFE = DEFAULT
  pred CUDNN = CAFFE
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Gen.Caffe.ReLUParameter.Engine"
...
instance P'.ReflectEnum Engine where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".caffe.ReLUParameter.Engine") ["Gen"] ["Caffe", "ReLUParameter"] "Engine")
      ["Gen", "Caffe", "ReLUParameter", "Engine.hs"]
